							...
							<div className="w-full p-4 flex cm-mobile-make-payment-topic">
								...
								<div className="pl-4 w-full">
									...
									<p className="pt-0 flex justify-between w-full">
										<p className="font-medium text-[#D3D4D4] text-sm c-elipses">
											By {paymentLink.creator_id ? `${paymentLink.creator_id.firstname} ${paymentLink.creator_id.lastname}` : 'Nill'}
										</p>
						...
						{tab === 1 ? (
							<div className="absolute cm-mobile-make-payment-panel">
								...
								<div className="p-6 flex flex-col items-between justify-between" style={{ minHeight: '90%' }}>
									<div className="c-bg-primary-light pt-4 mb-3 text-center bounce " onClick={() => setTab(2)}>
										{' '}
										Make New Payment{' '}
									</div>
									<div className="pt-4 mb-12 top-section" onClick={() => setTab(3)}>
										{' '}
													...
													{paymentLink.form.map((link, index) => (
														<Grid item xs={12} key={index}>
															<div className="flex flex-col space-y-3 mb-4">
																<label for={'for' + index + 1} className="text-sm font-bold block mt-0 mb-0 text-gray-700">
																	{link.field_name}
																</label>
																{link.field_type === 'text' ? (
																	<input
																		required
																		placeholder={link.field_name}
																		name={link.field_name + index}
																		onChange={(e) => handleFieldChanges(e, index)}
																		className="pb-2 px-4 w-full outline-none c-text-input"
																	/>
																) : (
																	<select
																		id={'for' + index + 1}
																...
																{link.error ? <small className="c-pay-error">{link.error}</small> : ''}
															</div>
														</Grid>
													))}
												</Grid>
						...
						{tab === 3 ? (
							<div className="absolute cm-mobile-make-payment-panel info">
								...
								<div className="p-6 flex flex-col items-between justify-between" style={{ minHeight: '90%' }}>
													...
													<Grid item xs={12}>
														<div className="flex flex-col space-y-3 mb-4">
															...
															{unique_object.field_type === 'text' ? (
																<input
																	required
																	placeholder={unique_object.field_name}
																	name={unique_object.field_name}
																...
																<select id={'for'} placeholder={unique_object.field_name} name={unique_object.field_name} className="pb-2 px-4 w-full outline-none c-text-input">
																	...
																	{unique_object.options.map((option, i) => (
																		<option key={option + i} onChange={(e) => setUniqueData(e.target.value)} value={option}>
																			{option}{' '}
																		</option>
																	))}
																</select>
															)}
															{unique_object?.error ? <small className="c-pay-error">{unique_object?.error}</small> : ''}
														</div>
													</Grid>
												</Grid>
											</div>
									...
									<button className="cm-buttom" onClick={() => verifyPayment()}>
										...
										<div className="pt-12">
											...
											<div className="">
												...
												<div className="relative pt-6 pb-4 w-full">
													<p className="text-[#315a5a] font-medium text-sm uppercase">{paymentLink.expires_at && 'Expiry Date'}</p>
										...
										<div className="hidden md:block">
											<Dialog open={openModal} onClose={handleCloseModal}>
												<div className="p-4 hidden md:block">
													<DialogTitle>
														<span className="font-bold text-xl"> {'Select An Action'} </span>
													</DialogTitle>
													<DialogContent>
										...
										<div className="w-full">
											<form className="w-full">
												<div className="flex justify-end items-end">
													...
													<div>
														<small className="text-sm text-[#2A86F2] status-pill c-status-border-pill capitalize">
															{paymentLink.status} {paymentLink.expires_at && ' - ' + moment(paymentLink.expires_at).format('dddd, DD MMMM YYYY')}
														</small>
													</div>
												</div>
														...
														<div className="py-2">
															...
															<div className="py-4">
																...
																<span className=" pl-4 text-sm ">
																	{' '}
																	I have paid already,
																	<span className="text-underline  cursor-pointer text-green-700 hover:text-green-900 font-bold " onClick={(e) => handleOpenModal()}>
																		{' '}
																		Verify Payment{' '}
																	</span>
																</span>
																...
																>
																	{({ initializePayment }) => (
																		<button className="hidden" disabled={loading ? true : false} ref={paystackButtonRef} onClick={(e) => openPaystack(e, initializePayment)}>
														...
														<div className="py-2">
															...
															<div>
																...
																<span className=" pl-4 text-sm ">
																	{' '}
																	Not paid yet?
																	<span className="text-underline  text-base cursor-pointer text-green-700 hover:text-green-900 font-bold " onClick={(e) => handleOpenModal()}>
																		{' '}
																		Make a New Payment{' '}
																	</span>
																</span>
